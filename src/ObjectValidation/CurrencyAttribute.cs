    ...
    public class CurrencyAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            if (value is not string currency)
                return new(
                    ErrorMessage ?? (validationContext.MemberName == null ? $"Currency code value as {typeof(string)} expected" : $"{validationContext.MemberName}: Currency code value as {typeof(string)} expected"),
            ...
            switch (Format)
            {
                case CurrencyFormats.AlphabeticCode:
                    if (!CurrencyCodes.Known.ContainsKey(currency))
                        return new(
                            ErrorMessage ?? (validationContext.MemberName == null ? $"Invalid currency code value" : $"{validationContext.MemberName}: Invalid currency code value"),
                            validationContext.MemberName == null ? null : new string[] { validationContext.MemberName }
                            );
                    break;
                ...
                default:
                    return new(
                        ErrorMessage ?? (validationContext.MemberName == null ? $"Invalid currency format configured" : $"{validationContext.MemberName}: Invalid currency format configured"),
                        validationContext.MemberName == null ? null : new string[] { validationContext.MemberName }
                        );
            }
            return null;
