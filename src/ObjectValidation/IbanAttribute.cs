    ...
    public class IbanAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            if (value is not string iban)
                return new(
                    ErrorMessage ?? (validationContext.MemberName == null ? $"IBAN value as {typeof(string)} expected" : $"{validationContext.MemberName}: IBAN value as {typeof(string)} expected"),
            ...
            if (!SwiftValidation.ValidateIban(Normalize ? SwiftValidation.Normalize(iban) : iban))
                return new(
                    ErrorMessage ?? (validationContext.MemberName == null ? $"Invalid IBAN value" : $"{validationContext.MemberName}: Invalid IBAN value"),
                    validationContext.MemberName == null ? null : new string[] { validationContext.MemberName }
                    );
            return null;
        }
